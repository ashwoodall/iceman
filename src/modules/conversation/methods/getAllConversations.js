...
const getAllConversations = (req, res, next) => {
  ...
  return db.task(tasks => {
    return tasks.map('SELECT id, initiator_id, recipient_id FROM ohhi_conversation WHERE recipient_id=$1 OR initiator_id=$1 AND is_deleted=false ORDER BY updated_at DESC', [id], conversation => {
      ...
      return tasks.one('SELECT id, first_name, last_name, profile_picture FROM ohhi_user WHERE id=$1', [other], user => {
        ...
        return tasks.one('SELECT * FROM ohhi_message WHERE convo_id=$1 AND is_deleted=false ORDER BY created_at DESC LIMIT 1', [conversation.id], message => {
