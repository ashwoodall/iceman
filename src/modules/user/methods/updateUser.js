...
const updateUser = (req, res, next) => {
  ...
  const batchQueries = () => {
    db.tx(transaction => {
      const queries = [
        transaction.one('UPDATE ohhi_user SET first_name=$1, last_name=$2, age=$3, hometown=$4, profile_picture=$5, introduction=$6, has_kids=$7, has_pets=$8, number_of_kids=$9, about_pets=$10, is_service_member=$11, current_station=$12, facebook=$13, twitter=$14, instagram=$15, pinterest=$16, completed_profile=$17 WHERE id=$18 RETURNING *', [
          first_name,
          last_name,
          age,
          hometown,
  ...
  const joinResults = () => {
    return db.tx(transaction => {
      const queries = [
        transaction.one('SELECT first_name, last_name, age, hometown, profile_picture, introduction, has_kids, has_pets, number_of_kids, about_pets, is_service_member, current_station, facebook, twitter, instagram, pinterest, completed_profile from ohhi_user WHERE id=$1', [id]),
